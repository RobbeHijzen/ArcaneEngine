...
void VersusLevel::Load(Scene& scene)
{
...
void VersusLevel::AddBackgroundImage(Scene& scene)
{
...
AE::GameObject* VersusLevel::AddGalaga(Scene& scene)
{
...
void VersusLevel::AddSpawnerManager(AE::Scene& scene, AE::GameObject* galaga)
{
	...
	auto beamCommand{std::make_unique<BossGalagaBeamCommand>(go.get())};
	auto diveCommand{std::make_unique<BossGalagaDiveCommand>(go.get())};
	auto shootCommand{std::make_unique<BossGalagaShootCommand>(go.get())};

	go->AddObserver(std::move(std::make_unique<VersusSpawnerManagerObserver>(spawnerManagerComp.get(), beamCommand.get(), diveCommand.get(), shootCommand.get())));

	InputManager::GetInstance().BindActionGP(0, INPUT_GAMEPAD_B, AE::InputType::IsDownThisFrame, std::move(beamCommand));
	InputManager::GetInstance().BindActionGP(0, INPUT_GAMEPAD_X, AE::InputType::IsDownThisFrame, std::move(diveCommand));
	InputManager::GetInstance().BindActionGP(0, INPUT_GAMEPAD_A, AE::InputType::IsDownThisFrame, std::move(shootCommand));
...
void VersusLevel::CreateSurroundingHitBoxes(AE::Scene& scene)
{
